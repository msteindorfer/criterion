...
public final class CalculateFootprintsHeterogeneous {
	...
	static {
		/*
		 * http://stackoverflow.com/questions/1518213/read-java-jvm-startup-
		 * parameters-eg-xmx
		 */
...
//		writeToFile(Paths.get(userHome, userHomeRelativePath, "map_sizes_heterogeneous_exponential_" + memoryArchitecture + "_latest.csv"),
//					appendToFile,
//					createExponentialRangeWithIntermediatePoints().stream()
//									.flatMap(size -> createLinearRange(0, 5, 1).stream().flatMap(run -> testOneConfiguration(size, run).stream())) 
//									.collect(Collectors.toList()));
//
//		writeToFile(Paths.get(userHome, userHomeRelativePath, "map_sizes_heterogeneous_exponential_" + memoryArchitecture + "_primitive_latest.csv"),
//						appendToFile,
//						createExponentialRangeWithIntermediatePoints().stream()
//						.flatMap(size -> createLinearRange(0, 5, 1).stream().flatMap(run -> testOnePrimitiveConfiguration(size, run).stream())) 
//						.collect(Collectors.toList()));		
		
//		writeToFile(Paths.get(userHome, userHomeRelativePath, "map_sizes_heterogeneous_exponential_" + memoryArchitecture + "_latest.csv"), appendToFile,
...
//		writeToFile(Paths.get(userHome, userHomeRelativePath, "map_sizes_heterogeneous_exponential_" + memoryArchitecture + "_primitive_latest.csv"), appendToFile,
...
//		writeToFile(Paths.get(userHome, userHomeRelativePath, "map_sizes_heterogeneous_tiny.csv"), appendToFile,
//				createLinearRange(0, 101, 1).stream()
//						.flatMap(size -> testOneConfiguration(size).stream()).collect(Collectors.toList()));
//		
//		writeToFile(Paths.get(userHome, userHomeRelativePath, "map_sizes_heterogeneous_small.csv"), appendToFile,
//				createLinearRange(0, 10_100, 100).stream()
//						.flatMap(size -> testOneConfiguration(size).stream()).collect(Collectors.toList()));
//
//		writeToFile(Paths.get(userHome, userHomeRelativePath, "map_sizes_heterogeneous_medium.csv"), appendToFile,
//				createLinearRange(10_000, 101_000, 1_000).stream()
//						.flatMap(size -> testOneConfiguration(size).stream()).collect(Collectors.toList()));
//
//		writeToFile(Paths.get(userHome, userHomeRelativePath, "map_sizes_heterogeneous_large.csv"), appendToFile,
...
//						createAndMeasureTrieSetMultimap(ValueFactoryFactory.VF_SCALA, size, multimapValueSize, stepSizeOneToOneSelector, run, preset),
//						createAndMeasureTrieSetMultimap(ValueFactoryFactory.VF_CLOJURE, size, multimapValueSize, stepSizeOneToOneSelector, run, preset),		
...
//						, createAndMeasureTrove4jTIntIntHashMap(data, size, 0, preset)
	...
	public static List<String> testOnePrimitiveConfiguration(int size, int run) {		
		...
		final MemoryFootprintPreset preset = MemoryFootprintPreset.RETAINED_SIZE_WITH_BOXED_INTEGER_FILTER;
...
//	}
	...
	public static String createAndMeasureGuavaImmutableMap(final Object[] data, int elementCount, int run,
			MemoryFootprintPreset preset) {
		com.google.common.collect.ImmutableMap.Builder<JmhValue, JmhValue> ysBldr = com.google.common.collect.ImmutableMap
				.builder();
		...
		return measureAndReport(ys, "com.google.common.collect.ImmutableMap", DataType.MAP, Archetype.IMMUTABLE, false,
	...
	public static String createAndMeasureGuavaImmutableSetMultimap(final Object[] data, int elementCount, int run,
			MemoryFootprintPreset preset) {
		com.google.common.collect.ImmutableSetMultimap.Builder<JmhValue, JmhValue> ysBldr = com.google.common.collect.ImmutableSetMultimap
				.builder();
		...
		return measureAndReport(ys, "com.google.common.collect.ImmutableSetMultimap", DataType.SET_MULTIMAP,
				Archetype.IMMUTABLE, false, elementCount, run, preset);
	}

	public static String createAndMeasureTrieMapHeterogeneous_asMap(final Object[] data, int elementCount, int run,
			MemoryFootprintPreset preset) {
		TrieMap_5Bits_Heterogeneous_BleedingEdge ys = (TrieMap_5Bits_Heterogeneous_BleedingEdge) TrieMap_5Bits_Heterogeneous_BleedingEdge
				.of();
	...
	public static String createAndMeasureTrieMapHeterogeneous_asMultimap(final Object[] data, int elementCount, int run,
			MemoryFootprintPreset preset) {
		TrieMap_5Bits_Heterogeneous_BleedingEdge ys = (TrieMap_5Bits_Heterogeneous_BleedingEdge) TrieMap_5Bits_Heterogeneous_BleedingEdge
				.of();
		...
		return measureAndReport(ys, shortName, DataType.SET_MULTIMAP, Archetype.PERSISTENT, false, elementCount, run,
				preset);
	}	

	public static String createAndMeasurePersistentMap(ValueFactoryFactory valueFactoryFactory, int elementCount, int run, MemoryFootprintPreset preset) {
		try {
	 ...
	 */
	@Deprecated
	public static String createAndMeasureTrieMap(ValueFactoryFactory valueFactoryFactory, int elementCount, int run, MemoryFootprintPreset preset) {
		try {
	...
	public static String createAndMeasureTrieSetMultimap(ValueFactoryFactory valueFactoryFactory, int elementCount,
			int multimapValueSize, int stepSizeOneToOneSelector, int run, MemoryFootprintPreset preset) {
		try {
			final Object setMultimapInstance = JmhSetMultimapBenchmarks.generateSetMultimap(
					valueFactoryFactory.getInstance(), ElementProducer.PDB_INTEGER, false, elementCount,
					multimapValueSize, stepSizeOneToOneSelector, run);

			return measureAndReport(setMultimapInstance, valueFactoryFactory.name(), DataType.SET_MULTIMAP,
					Archetype.PERSISTENT, false, elementCount, run, preset);
		} catch (Exception e) {
			e.printStackTrace();
		}
...
//				assert ys.containsKey(v);				
	...
	public static String createAndMeasureTrove4jTIntIntHashMap(final Object[] data, int elementCount, int run,
		...
		return measureAndReport(ys, "gnu.trove.map.hash.TIntIntHashMap", DataType.MAP, Archetype.MUTABLE, false, elementCount, run, preset);
	}
...
//							Predicates.instanceOf(JmhValue.class), Predicates.instanceOf(PureInteger.class)));
	...
	private static String measureAndReport(final Object objectToMeasure, final String className, DataType dataType,
		...
		final String statString = String.format("%d (%d@%d)\t %60s\t\t %s", size, run, memoryInBytes, className, memoryFootprint);
