...
public class JmhMapBenchmarks {
	...
	public void setUp() throws Exception {
		...
		case RANDOM: {
			...
			for (int i = 0; i < CACHED_NUMBERS_SIZE; i++) {
				cachedNumbers[i] = valueFactory.integer(randForOperations
						.nextInt());
			}
		}
		...
		case MATCH: {
			...
			for (int i = 0; i < CACHED_NUMBERS_SIZE; i++) {
				...
				boolean found = false;
				while (!found) {
					final IValue candidate = valueFactory.integer(anotherRand
							.nextInt());
	...
	protected void setUpTestMapWithRandomContent(int size, int run)
			throws Exception {
		 ...
		 */
		int existingValueIndex = new Random(seedForThisTrial + 13)
				.nextInt(size);
	...
	public void timeEntryIteration(Blackhole bh) {
		for (Iterator<java.util.Map.Entry<IValue, IValue>> iterator = testMap
	...
	public void timeInsert(Blackhole bh) {
		for (int i = 0; i < CACHED_NUMBERS_SIZE; i++) {
			bh.consume(testMap.put(cachedNumbersNotContained[i],
					VALUE_NOT_EXISTING));
