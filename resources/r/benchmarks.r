...
calculateMemoryFootprintOverhead <- function(requestedDataType, dataStructureOrigin) {
  ...
  ##
  dss32_fileName <- paste(paste("map-sizes-and-statistics", "32bit", timestamp, sep="-"), "csv", sep=".")
  dss32_stats <- read.csv(dss32_fileName, sep=",", header=TRUE)
  dss32_stats <- within(dss32_stats, arch <- factor(32))
  #
  dss64_fileName <- paste(paste("map-sizes-and-statistics", "64bit", timestamp, sep="-"), "csv", sep=".")
...
benchmarksFileName <- paste(paste("results.all", timestamp, sep="-"), "log", sep=".")
