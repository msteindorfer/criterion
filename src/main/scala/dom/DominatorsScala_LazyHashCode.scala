...
import dom.AllDominatorsRunner.{DATA_SET_SINGLE_FILE_NAME, LOG_BINARY_RESULTS, LOG_TEXTUAL_RESULTS}
import dom.DominatorsScala_LazyHashCode._
import io.usethesource.vallang.io.StandardTextWriter
import io.usethesource.vallang.io.old.{BinaryValueReader, BinaryValueWriter}
import io.usethesource.vallang.{IConstructor, IMap, ISet, ITuple, IValue}
import org.openjdk.jmh.infra.Blackhole
import org.rascalmpl.interpreter.utils.Timing

import scala.collection.JavaConverters.{asScalaBufferConverter, asScalaSetConverter, iterableAsScalaIterableConverter}
import scala.collection.immutable.{HashMap_LazyHashCode, HashSet_LazyHashCode}
  ...
  def calculateDominators(graph: HashSet_LazyHashCode[ITuple]): HashMap_LazyHashCode[IConstructor, HashSet_LazyHashCode[IConstructor]] = {
    ...
    while (prev != dom) {
      ...
      for (n <- nodes) {
        ...
        val intersected = if (sos == null || sos.isEmpty || sos.contains(HashSet_LazyHashCode.empty)) HashSet_LazyHashCode.empty[IConstructor] else sos reduce {
          _ intersect _
        }
